...
##---- Readin ----
heath.center <- read_excel('02_Inputs/2019年中国卫生院数据库（含6123街道卫生院）.xlsx', sheet = 2)# %>% 
  # mutate(flag = 1) %>% 
  # bind_rows(read_excel('02_Inputs/2019年中国卫生院数据库（含6123街道卫生院）.xlsx', sheet = 3)) %>% 
  # mutate(flag = if_else(is.na(flag), 0, flag))
# heath.center <- read_excel('02_Inputs/2019年中国卫生院数据库clean V2_闽侯.xls')
